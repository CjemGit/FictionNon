...
settings2 = json.load(open('settings.json'))

for settings in settings2:

    Architecture = settings2[settings].get("Architecture","LSTM")

    Spec = settings2[settings].get("Spec")

    maxSeqLength = settings2[settings].get("maxSeqLength", 300)
    maxSeqLength = int(maxSeqLength)

    batchSize = settings2[settings].get("batchSize", 24)
    batchSize = int(batchSize)

    lstmUnits = settings2[settings].get("lstmUnits",64)
    lstmUnits = int(lstmUnits)

    dropout = settings2[settings].get("output_keep_prob", 0.75)
    dropout = float(dropout)

    rewrite = settings2[settings].get("rewrite", False)
    rewrite = bool(rewrite)

    vectorpath = settings2[settings].get("vectorpath","C:\\Users\\Clembo\\Desktop\\Thesis\\Data\\GoogleNews-vectors-negative300.bin")
    ...
    if Architecture == "LSTM":
        ...
        import numpy as np
        wordVectors = load_embedding_vectors_word2vec(vocabulary, vectorpath, True)
        ...
        #http://localhost:6006/
        timer.to_csv('models/' + Spec + 'timer.csv')

        print('completed' + Spec)
